...
const sortNumbersByLength = (arr) => {
  return arr.sort((a, b) => {
    let aElem = a.toString().split('.').join('').length;
    let bElem = b.toString().split('.').join('').length;
    return aElem - bElem;
  })
};
...
const sortPeopleBetter = (arr) => {
  let res = arr.sort((a,b)=>{
    let aVal = a.lastName + a.firstName + a.age;
    let bVal = b.lastName + b.firstName + b.age;
    if (aVal > bVal) {
      return 1;
    } else if (aVal < bVal) {
      return -1;
