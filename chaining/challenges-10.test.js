...
const count = (target, input) => {
  return input.flat().filter(elem => elem === target).length
  // Solution code here...
};
...
const totalSum = (input) => {
  return input.flat().reduce((acc, curr) => { return acc + curr }, 0)
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => input.map(arr => arr.filter(elem => elem % 5 === 0 && typeof elem === 'number').map(elem => Math.pow(2, elem)));
...
let findMaleAndFemale = (data) => {
  return data.filter(elem => { return elem.gender.match(/male/g) }).map(elem => elem.name).join(' and ')
};
...
let findShortest = (data) => {
  ...
  data.map(elem => {
    if (res === undefined) {
      ...
      res = parseInt(elem.height) < parseInt(res.height) ? elem : res;
    }
  })
  return res.name
  // Solution code here...
