...
const findAnything = (arr, target) => {
  let res = [];
  arr.forEach(word => {
    if (word.includes(target)) res.push(word);
  })
  return res
  // Solution code here...
};
...
const findEvery = (arr, target) => {
  return arr.length === findAnything(arr, target).length;
};
...
const unenrollBrook = (arr) => {
  return arr.map(classArr => {
    return classArr.filter((student) => {
