...
const filterStringsWithVowels = (arr) => {
  return arr.filter(el => el.search(/[aeiou]/g) !== -1);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(el => forbiddenValues.indexOf(el) === -1)
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(el => el.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  // console.log(arr);
  return getBaseStatGreaterThan(arr, minBaseStat).map(el => el.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(el => !el.children)
  // Solution code here...
};
